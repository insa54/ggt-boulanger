...
class Param extends Exception
{
    ...
    {
        return (isset($_SESSION[_USER_]->id) && ($_SESSION[_USER_]->id > 0)) ? true : false;
    }

    public function authenticated($baseuri = "/connexion")
    {
        if ($this->online() && isset($_SESSION[_USER_]->premiere_connexion) && ($_SESSION[_USER_]->premiere_connexion == 1)) {
    ...
    {
        return ($this->online() && $_SESSION[_USER_]->code_type_utilisateur === $who) ? true : false;
    }
    ...
    public function _partial_disconnect($path = HTTP_PATH, $app)
    {
        unset($_SESSION[$app]);
        $this->redirect($path);
    }
