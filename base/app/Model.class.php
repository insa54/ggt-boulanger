...
class Model extends Param
{
    ...
    public function getClause()
    {
        ...
        // if (count($a) > 0) {
        //     foreach ($a as $s) {
        //         $clause .= " AND " . $s . ".supprime=-1";
        //     }
        // } else {
        //     $clause .= ' AND supprime=-1';
        // }
    ...
    public function getData($release = false, $track = false, $fetchWith = null, $getClause = true)
    {
        ...
        $requete .= ($this->getClause() != "" && $getClause == true) ? " WHERE " . $this->getClause() : "";
    ...
    {
        try {
            $requete = "UPDATE " . $this->getTable() . " SET supprime=1, date_suppression=NOW()";
        } catch (Exception $ex) {
            die($ex->getMessage());
        }
    ...
    public function freeQuery($requete, $track = false)
    {
        try {
            $requete .= $this->getClause() != "" ? " WHERE " . $this->getClause() : "";
        } catch (Exception $ex) {
    ...
    public function updateOne($donnees = [], $index = "id", $valeurIndex = null, $track = false)
    {
        ...
        $requete .= ($this->getClause() != '1=1 AND supprime=-1') ? " WHERE " . $this->getClause() : (" WHERE $index = " . (!empty($valeurIndex) ? "'" . $valeurIndex . "'" : ":$index"));
    ...
    public function getElementType($typesCommuns = [], $typesByAgences = [], $pour = '', $nomBD = '')
    {
        ...
        if (count($typesByAgences) > 0) {
            ...
            $nomBD = !empty($nomBD) ? $nomBD : $_SESSION[_USER_]->nom_bd;
    ...
    public function getDeptByCp($cp)
    {
        ...
        $this->setClause("num IN (SELECT num_departement from crm_np_ville WHERE code_postal=" . $cp . " AND supprime = -1)");
        $this->setOrderBy("id");
    ...
    public function checkColumExist($db, $column)
    {

        $requete = "SHOW COLUMNS FROM " . $db . " LIKE '".$column."'";
    ...
    public function addColumToTable($table, $column, $type = "VARCHAR(255)")
    {

        $requete = "ALTER TABLE " . $table . " ADD ".$column." ".$type." NULL";
