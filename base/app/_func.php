...
function JSONGen($model, $route, $nom_bd)
{
    ...
    $fp = fopen($route . STORAGE_PATH . 'agences/' . $nom_bd . '/config.json', 'w');
...
function JSONGenFromCommun($model, $route, $toGen = 'villes')
{
    ...
    $fp = fopen($route . STORAGE_PATH . 'agences/' . $toGen . '.json', 'w');
    fwrite($fp, json_encode($donnees));
    fclose($fp);
}
...
function rappelWeek($model)
{
    ...
    $model->release();
    $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_tache t, crm_np_utilisateur u');
    $model->setChamp('*');
    $model->setLimit(10);
    $model->setGroupBy('t.id');
    $model->setOrderBy('t.date_debut ASC');

    $model->setClause('t.code_type_tache="RAPPEL" AND t.attribue_a=u.id AND DATE(t.date_echeance) >= CURDATE() AND t.date_echeance < CURDATE() + INTERVAL 7 DAY AND (t.attribue_a =' . $_SESSION[_USER_]->id . ' OR t.id in (SELECT id_tache from ' . $_SESSION[_USER_]->nom_bd . '.crm_np_tache_concerne where id_concerne=' . $_SESSION[_USER_]->id . ' AND code_source_concerne="UTILISATEUR" AND supprime = -1))');
...
function JSONGenAffaireChamp($model, $route, $nom_bd, $nom_doss)
{
    ...
    $fp = fopen($route . STORAGE_PATH . 'agences/' . $nom_doss . '/liste_champs_affaires.json', 'w');
    fwrite($fp, json_encode($donnees));
}

function genLEFJSON($model, $route, $nom_bd = '')
{
    $nom_b = ($nom_bd != '') ? PREFIXE_BD . $nom_bd : $_SESSION[_USER_]->nom_bd;
    $nom_bd = ($nom_bd != '') ? $nom_bd : $_SESSION[_USER_]->nom_dossier;
    $model->release();
    $model->setChamp("*");
    ...
    $fp = fopen($route . STORAGE_PATH . 'agences/' . $nom_bd . '/liste_element_form.json', 'w');
...
function getDBName($model, $idfk, $fkTablename = CRM_NP_AGENCES)
{
    $model->release();
    $model->setTable(CRM_NP_AGENCES);
    $model->setClause('id = ' . (($fkTablename === CRM_NP_AGENCES) ? $idfk : '(SELECT id_agence FROM ' . $fkTablename . ' WHERE id = "' . $idfk . '") actif=1'));
...
function prepareMail($route, $sujet, $message, $params = [])
{
    ...
    $info_agence['agence']['logo'] = STORAGE_PATH . 'agences/agenceModele/logo.svg';
    ...
    if (isset($params[0]) && count($params[0])) {
        ...
        if (!empty($info_agence['agence']['logo'])) {
            $info_agence['agence']['logo'] = STORAGE_PATH . 'agences/' . $info_agence['agence']['nom_dossier'] . '/' . $info_agence['agence']['logo'];
        }
        ...
        $expediteur = $_SESSION[_USER_]->email;
    }
...
function isUnique($model, $table, $clause, $field = "id", $kind = 'count')
{
    ...
    return ($kind === 'field') ? $model->getData() : $model->getCount($field);
}
...
function agencies($model)
{
    # Récupération des agences
    $model->release();
    $model->setTable("crm_np_agences");
    $model->setChamp("id, raison_sociale, enseigne, nom_bd");
    $model->setClause("actif = 1");
    $agences = $model->getData(true, false, 'id');
...
UID:" . $donnees->id . "
ORGANIZER;CN=" . $_SESSION[_USER_]->nom . " " . $_SESSION[_USER_]->prenom . ":mailto:" . $_SESSION[_USER_]->email . "
" . $attendee . "
CLASS:PRIVATE
...
END:VCALENDAR";
    ...
    /* return "BEGIN:VCALENDAR
      ...
      BEGIN:VEVENT
      ORGANIZER;CUTYPE=INDIVIDUAL;ROLE=REQ-PARTICIPANT;PARTSTAT=ACCEPTED;CN=" . $_SESSION[_USER_]->nom . " " . $_SESSION[_USER_]->prenom . ";X-NUM-GUESTS=0:MAILTO:" . $_SESSION[_USER_]->email . "
      CLASS:PRIVATE
      STATUS:CONFIRMED
      DESCRIPTION:" . $donnees->objet . "
      TRANSP:OPAQUE
...
function maselection($model)
{
    ...
    $model->release();
    $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_affaire a LEFT JOIN crm_np_ville v ON a.id_ville = v.id, crm_np_liste_element_form lef, ' . $_SESSION[_USER_]->nom_bd . '.crm_np_affaire_type at');
    ...
    $clause = "a.id_type_affaire = at.id  AND a.code_type_transaction = lef.code AND a.id IN (SELECT id_affaire FROM " . $_SESSION[_USER_]->nom_bd . ".crm_np_wish_list WHERE email = '" . $_SESSION[_USER_]->email . "' AND supprime = -1) AND a.actif = 1";
    ...
    $config = JSONFileToArray($route . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/config.json');
...
{
    $files = (is_dir($route . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/images/affaires/affaire_' . $id)) ? array_slice(scandir($route . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/images/affaires/affaire_' . $id), 2) : [];
    $photo = "";
    ...
    if ($photo != "") {
        $photo = HTTP_PATH . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/images/affaires/affaire_' . $id . '/' . $photo;
    } else {
        if (count($files) > 0) {
            $photo = HTTP_PATH . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/images/affaires/affaire_' . $id . '/' . $files[0];
        }
    }
    $config = JSONFileToArray($route . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/config.json');
    $photo = !file_exists(str_replace(HTTP_PATH, $route, $photo)) ? HTTP_PATH . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/' . $config['agence']['logo'] : $photo; #Ajouter par Ousmane le 27/02/2021
    return $photo;
}
...
{
    $model->release();
    $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_wish_list');
    $model->setChamp("id_affaire");
    $model->setClause("email = '" . $_SESSION[_USER_]->email . "' AND id_affaire IN (SELECT id FROM " . $_SESSION[_USER_]->nom_bd . ".crm_np_affaire WHERE actif = 1 AND supprime = -1)");
...
function encompromis($model, $field = 'id_affaire')
{
    ...
    $model->release();
    $model->setChamp($field);
    $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_compromis_information');
...
function manueltoelectronique($model, $table, $datas)
{
    $current = 1;
    foreach ($datas as $data) {
        $model->setTable($_SESSION[_USER_]->nom_bd . '.' . $table);
...
{
    if ($_SESSION[_USER_]->nom_bd === $agence) {
        dump($datas);
        exit();
    }
}

function dumpforsuper($datas, $exit = true)
{
    global $model;
    if ((bool) $model->connected("SUPERADMINISTRATEUR") === true) {
        dump($datas);
        if ($exit)
            exit();
    }
...
function get_dept_autorises($model)
{
    ...
    $model->setClause("id = " . $_SESSION[_USER_]->id);
...
function alertes($model, $id, $lef, $csc = 'AFFAIRE')
{
    ...
    $clausealerte = "id IN (SELECT id_tache FROM " . $_SESSION[_USER_]->nom_bd . ".crm_np_tache_concerne WHERE code_source_concerne = '" . $csc . "' AND id_concerne = " . $id . ")";
    if ($model->connected('NEGOCIATEUR')) {
        $clausealerte .= " AND (attribue_a = " . $_SESSION[_USER_]->id . " OR cree_par = " . $_SESSION[_USER_]->id . ")";
    }
    if ($csc === 'UTILISATEUR') {
        $clausealerte = "((id IN (SELECT id_tache FROM " . $_SESSION[_USER_]->nom_bd . ".crm_np_tache_concerne WHERE code_source_concerne = '" . $csc . "' AND id_concerne = " . $id . ")) OR attribue_a = " . $id . ")";
    }
    $model->release();
...
function docs($route, $folder, $docsinfo = false, $splitname = false, $order = false)
{
    ...
    $docpathname = getdir($route . STORAGE_PATH, "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/$folder/");
    ...
    foreach ($docs as $i => $doc) {
        if (is_dir($route . STORAGE_PATH . "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/$folder/$doc")) {
            continue;
        }
        $info = pathinfo($doc);
        $docname = HTTP_PATH . STORAGE_PATH . "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/$folder/$doc";
        $returned[$i] = ['realpath' => $route . STORAGE_PATH . "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/$folder/$doc", 'extension' => $info['extension'], 'name' => $info['filename'], 'fullname' => $docname, 'size' => getFileSize($docpathname . $doc)];
...
{
    $table_commun = ['crm_np_acces_module', 'crm_np_agences', 'crm_np_mail_type', 'crm_np_lettre_type', 'crm_np_code_naf', 'crm_np_department', 'crm_np_droits', 'crm_np_liste_element_form', 'crm_np_utilisateur', 'crm_np_ville'];

    $nomBD = empty($nomBD) ? $_SESSION[_USER_]->nom_bd : $nomBD;
...
function _get_family_affaire($model, $current_affaire, $family, $defaultSelected, $action, $caller = '', $clauseMaJ = '', $nomBD = '', $for = 'VIEW')
{
    ...
    if ($action === "ajout" || ($action === "ajoutprospect" && $caller !== "ajax")) {
        ...
        $clause_for_field = 'fc.utilise_pour="' . strtoupper($for) . '" AND vc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND fc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND vc.types_affaires_autorises like "%#' . $defaultSelected['id_type_affaire'] . '#%" AND vc.id_famille' . $for_suffix . '=fc.id  AND fc.rang= ' . $family . $isprospect;
    } elseif ($action === "detail") {

        $clause_for_field = 'fc.utilise_pour="' . strtoupper($for) . '" AND vc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND fc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND vc.types_affaires_autorises like "%#' . $defaultSelected['id_type_affaire'] . '#%" AND vc.id_famille' . $for_suffix . '=fc.id' . $isprospect;
    } elseif ($caller === "ajax") {
        ...
        $clause_for_field = $clauseMaJ . 'fc.utilise_pour="' . strtoupper($for) . '" AND vc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND fc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" ' . $clause_type_affaire . ' AND vc.id_famille' . $for_suffix . '=fc.id AND fc.id=' . $family . $isprospect;
    } else {

        if (isset($_SESSION['defaultSelected']['idFamille']) && (int) $_SESSION['defaultSelected']['idFamille'] > 0) {
            $clause_for_field = 'nom_champ NOT IN("code_type_transaction","id_type_affaire") AND fc.utilise_pour="' . strtoupper($for) . '" AND vc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND fc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND vc.types_affaires_autorises like "%#' . $defaultSelected['id_type_affaire'] . '#%" AND vc.id_famille' . $for_suffix . '=fc.id AND fc.id=' . $_SESSION['defaultSelected']['idFamille'] . $isprospect;
        } else {
            $clause_for_field = 'nom_champ NOT IN("code_type_transaction","id_type_affaire") AND fc.utilise_pour="' . strtoupper($for) . '" AND vc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND fc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND vc.types_affaires_autorises like "%#' . $defaultSelected['id_type_affaire'] . '#%" AND vc.id_famille' . $for_suffix . '=fc.id AND fc.rang=' . $family . $isprospect;
        }
    }

    $model->setTable($nomBD . '.crm_np_affaire_vis_famille_champ fc, ' . $nomBD . '.crm_np_affaire_vis_champ vc');
    $model->setClause($clause_for_hide_negociateur_field . $clause_for_field);
...
function getDetailAffaire($model, $id_function_affaire, $lef, $action = 'detail', $ignoreDroitAcces = false, $nomBD = '', $for = 'VIEW')
{

    global $acces_mandats_affaires_autres_negociateurs, $departements_autorises, $lefagence, $assigned;
    ...
    if ($ignoreDroitAcces === true) {
        ...
        $model->setClause('id = ' . $id_function_affaire . (($acces_mandats_affaires_autres_negociateurs) ? '' : ' AND attribue_a = ' . $_SESSION[_USER_]->id));
    }
    ...
    for ($i = 0; $i < count($famillesChamp); $i++) {
        if (($famillesChamp[$i]->source_donnees != "LIBRE") && ($famillesChamp[$i]->source_donnees != "") && ($famillesChamp[$i]->source_donnees != null)) {
            ...
            switch ($famillesChamp[$i]->source_donnees) {
                case $nomBD . '.crm_np_liste_element_form':
    ...
    # Récupérations la liste des vendeurs concernant l'affaire
    $model->release();
    $model->setTable($nomBD . '.crm_np_affaire_vendeur');
    $model->setClause('id_affaire=' . $id_function_affaire);
    $model->setChamp("*");
    $model->setOrderBy("id");
    $vendeurs = $model->getData(true);

    if (count($vendeurs)) {
        foreach ($vendeurs as $v) {
            $v->date_creation = textDate($v->date_creation);
            $v->date_naissance = textDate($v->date_naissance, '%d %b %Y');
            $v->date_naissance_conjoint = textDate($v->date_naissance_conjoint, '%d %b %Y');
            $v->code_type_profil_txt = (isset($lef['TYPE_PROFIL'][$v->code_type_profil]['libelle'])) ? $lef['TYPE_PROFIL'][$v->code_type_profil]['libelle'] : '';
            $v->code_civilite_txt = (isset($lef['CIVILITE'][$v->code_civilite]['libelle'])) ? $lef['CIVILITE'][$v->code_civilite]['libelle'] : '';
            $v->code_civilite_conjoint_txt = (isset($lef['CIVILITE'][$v->code_civilite_conjoint]['libelle'])) ? $lef['CIVILITE'][$v->code_civilite_conjoint]['libelle'] : '';

            if ((int) $v->id_ville > 0) {
                $model->release();
                $model->setTable("crm_np_ville");
                $model->setClause("id = " . $v->id_ville);
                $model->setChamp("nom_reel as ville");
                $vendeurville = $model->getData();
                $v->ville = isset($vendeurville[0]->ville) ? $vendeurville[0]->ville : "";
            }

            if ((int) $v->id_ville_conjoint > 0) {
                $model->release();
                $model->setTable("crm_np_ville");
                $model->setClause("id = " . $v->id_ville_conjoint);
                $model->setChamp("nom_reel as ville");
                $vendeurvilleconjoint = $model->getData();
                $v->ville_conjoint = isset($vendeurvilleconjoint[0]->ville) ? $vendeurvilleconjoint[0]->ville : "";
            }
        }
        $current_affaire['vendeurs'] = $vendeurs;
    } else {
        $current_affaire['vendeurs'] = [];
    }
...
function _get_family_affaire_impression($model, $current_affaire, $family, $defaultSelected, $action, $caller = '', $clauseMaJ = '')
{
    ...
    if (($action === "ajout") || ($action === "ajoutprospect" && $caller !== "ajax")) {
        ...
        $clause_for_field = 'fc.utilise_pour="IMPRESSION" AND vc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND fc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND vc.types_affaires_autorises like "%#' . $defaultSelected['id_type_affaire'] . '#%" AND vc.id_famille_impression=fc.id  AND fc.rang= ' . $family . $isprospect;
    } elseif ($action === "detail") {

        $clause_for_field = 'fc.utilise_pour="IMPRESSION" AND vc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND fc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND vc.types_affaires_autorises like "%#' . $defaultSelected['id_type_affaire'] . '#%" AND vc.id_famille_impression=fc.id' . $isprospect;
    } elseif ($caller === "ajax") {
        $clause_type_affaire = (isset($defaultSelected['id_type_affaire']) && (int) $defaultSelected['id_type_affaire'] > 0) ? 'AND vc.types_affaires_autorises like "%#' . $defaultSelected['id_type_affaire'] . '#%"' : '';
        $clause_for_field = $clauseMaJ . 'fc.utilise_pour="IMPRESSION" AND vc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" AND fc.code_type_transaction="' . $defaultSelected['code_type_transaction'] . '" ' . $clause_type_affaire . ' AND vc.id_famille_impression=fc.id AND fc.id=' . $family . $isprospect;
    } else {
    ...
    $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_affaire_vis_famille_champ fc, ' . $_SESSION[_USER_]->nom_bd . '.crm_np_affaire_vis_champ vc');
    $model->setClause($clause_for_hide_negociateur_field . $clause_for_field);
    $model->setChamp("fc.*,vc.*");
...
{
    global $route, $oSmarty, $assigned, $config;
    $liste = [];
    $lef = $model->getElementType(["CIVILITE", "TYPE_MANDAT", "TYPE_PROFIL", "LICENCE", "FORME_JURIDIQUE"], ["STATUT_CONTACT", "ORIGINE_ACHETEUR", "SCORING", "DIPLOME_ACHETEUR", "TYPE_PROFIL", "CESSION_AFFAIRE"]);
    $recherche_details = (function ($recherche) use ($model) {
        $model->release();
        $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_acheteur_recherche_detail');
        ...
        switch ($group) {
            ...
            case 'UTILISATEUR':
                ...
                for ($i = 0; $i < count($resultat); $i++) {
                    ...
                    $liste['UTILISATEUR'][$i]['photo_negociateur'] = (!empty($resultat[$i]->photo) && file_exists($route . STORAGE_PATH . "images/utilisateurs/" . $resultat[$i]->photo)) ? STORAGE_PATH . "images/utilisateurs/" . $resultat[$i]->photo : MED_PATH_BE . "/users/avatar-default-icon.png";
            ...
            case 'CONTACT':
                ...
                $champsConcernes = ['rcs_siret', 'raison_sociale', 'nom', 'prenom', 'adresse', 'cp', 'telephone', 'portable', 'email', 'login', 'societe', 'fonction', 'commentaire'];
                for ($i = 0; $i < count($resultat); $i++) {
                    $liste[$group][$i]['photo_contact'] = (!empty($resultat[$i]->photo) && file_exists($route . STORAGE_PATH . "agences/" . $_SESSION[_USER_]->nom_dossier . "/images/contacts/" . $resultat[$i]->photo)) ? STORAGE_PATH . "agences/" . $_SESSION[_USER_]->nom_dossier . "/images/contacts/" . $resultat[$i]->photo : MED_PATH_BE . "/users/avatar-default-icon.png";
                    $liste[$group][$i]['id'] = $resultat[$i]->id;
                    $liste[$group][$i]['code_type_profil'] = $resultat[$i]->code_type_profil;
                    $liste[$group][$i]['code_type_profil_contact'] = $resultat[$i]->code_type_profil;
                    $liste[$group][$i]['civilite_contact'] = (isset($resultat[$i]->code_civilite) && !empty($resultat[$i]->code_civilite) && isset($lef["CIVILITE"][$resultat[$i]->code_civilite])) ? $lef["CIVILITE"][$resultat[$i]->code_civilite]['libelle'] : "";
                    $liste[$group][$i]['statut_contact'] = (isset($resultat[$i]->code_statut_contact) && !empty($resultat[$i]->code_statut_contact) && isset($lef["STATUT_CONTACT"][$resultat[$i]->code_statut_contact])) ? $lef["STATUT_CONTACT"][$resultat[$i]->code_statut_contact]['libelle'] : "";
                    $liste[$group][$i]['motdepasse_contact'] = $resultat[$i]->mdp;
                    foreach ($champsConcernes as $champ) {
                        $liste[$group][$i][$champ . '_contact'] = $resultat[$i]->$champ;
                    }
                    $liste[$group][$i]['ville_contact'] = '';
                    if ($resultat[$i]->id_ville > 0) {
                        $model->setTable(CRM_NP_VILLE);
                        $model->setChamp(VILLE_NOM);
                        $model->setClause(VILLE_ID . " = " . $resultat[$i]->id_ville);
                        $ville = $model->getData(true);
                        $liste[$group][$i]['ville_contact'] = (count($ville)) ? $ville[0]->nom : "";
                    }
                }
            ...
            case 'AFFAIRE':
                ...
                foreach ($valeur as $k => $id) {
                    $detail = getDetailAffaire($model, $id, $lef, 'lettremailtype');
                    if ($detail != false && is_array($detail)) {
                        foreach ($detail['champInput'] as $kchamp => $vchamp) {
                            if ($vchamp->nom_champ === 'vendeur') {
                                $detail['champInput'][$kchamp]->valeur_got = $detail['current_affaire']['vendeurs'];
                                continue;
                            }
                ...
                if (count($resultat)) {
                    ...
                    foreach ($resultat as $i => $affaire) {
                        foreach ($affaire as $kdet => $vdet) {
                            ...
                            # $avatardir = $route . STORAGE_PATH . "images/utilisateurs/";
                            if ($vdet->nom_champ === 'image_affaire') {
                                $files = (is_dir($route . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/images/affaires/affaire_' . $affaire['id'])) ? array_slice(scandir($route . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/images/affaires/affaire_' . $affaire['id']), 2) : [];
                                $filesnumber = count($files);
                                if ($filesnumber > 0) {
                                    for ($j = 0; $j < $filesnumber; $j++) {
                                        $files[$j] = HTTP_PATH . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/images/affaires/affaire_' . $affaire['id'] . '/' . $files[$j];
                                    }
                                }
                            ...
                            if ($vdet->nom_champ === 'attribue_a') {
                                ...
                                $liste[$group][$i][$variable_lexique . $suffixe]['photo_negociateur'] = (!empty($negociateur['photo']) && file_exists($route . STORAGE_PATH . "images/utilisateurs/" . $negociateur['photo'])) ? STORAGE_PATH . "images/utilisateurs/" . $negociateur['photo'] : MED_PATH_BE . "/users/avatar-default-icon.png";
                            ...
                            if ($vdet->nom_champ === 'vendeur') {
                                ...
                                foreach ($vdet->valeur_got as $kvendeur => $vendeur) {
                                    ...
                                    $liste[$group][$i][$variable_lexique . $suffixe][$kvendeur]['civilite_proprietaire'] = $vendeur->code_civilite_txt;
                                    $liste[$group][$i][$variable_lexique . $suffixe][$kvendeur]['civilite_conjoint_proprietaire'] = $vendeur->code_civilite_conjoint_txt;
            ...
            case 'VENDEUR':
                ...
                $model->setClause("id IN (" . implode(',', $valeur) . ")");
                $resultat = $model->getData();
                $liste[$group] = [];
                ...
                for ($i = 0; $i < count($resultat); $i++) {
                    ...
                    if ($resultat[$i]->id_ville_conjoint > 0) {
                        $model->setTable(CRM_NP_VILLE);
                        $model->setChamp(VILLE_NOM);
                        $model->setClause(VILLE_ID . " = " . $resultat[$i]->id_ville_conjoint);
                        $ville = $model->getData(true);
            ...
            case 'ACHETEURRECHERCHE':
                ...
                # On recupere les variables du lexique depuis la table visibilite champ affaire
                $model->release();
                $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_acheteur a,' . $_SESSION[_USER_]->nom_bd . '.crm_np_acheteur_recherche ar');
                $model->setChamp("a.*, ar.*");
                $clause = "a.id = ar.id_acheteur AND ar.id IN (" . implode(',', $valeur) . ")";
                $model->setClause($clause);
                $resultat = $model->getData(true, false);

                include($route . '/backend/composants/_rapprochement.php');
                ...
                for ($i = 0; $i < count($resultat); $i++) {
                    ...
                    foreach ($champsConcernesRecherche as $champ) {
                        ...
                        if ($champ === 'id_acheteur_associe' && !empty($resultat[$i]->id_acheteur_associe)) {
                            ...
                            $model->setClause('id IN (' . str_replace("#", "", str_replace("##", ",", $resultat[$i]->id_acheteur_associe)) . ')');
                        ...
                        if ($champ === 'attribue_a') {
                            ...
                            $liste[$group][$i]['photo_attributaire'] = (!empty($negociateur['photo']) && file_exists($route . STORAGE_PATH . "images/utilisateurs/" . $negociateur['photo'])) ? STORAGE_PATH . "images/utilisateurs/" . $negociateur['photo'] : MED_PATH_BE . "/users/avatar-default-icon.png";
                            if ($negociateur['id_ville'] > 0) {
                                $model->setTable(CRM_NP_VILLE);
                                $model->setChamp(VILLE_NOM);
                                $model->setClause(VILLE_ID . " = " . $negociateur['id_ville']);
...
function nperror()
{
    ...
    if (_env === "DEV" || (isset($_SESSION[_USER_]->code_type_utilisateur) && $_SESSION[_USER_]->code_type_utilisateur === "SUPERADMINISTRATEUR")) {
    ...
    if (isset($_SESSION[_USER_])) {
        $log = $route . STORAGE_PATH . "agences/" . $_SESSION[_USER_]->nom_dossier . "/errorlog.txt";
    } else {
        $log = $route . STORAGE_PATH . "agences/errorlog.txt";
    }
...
{
    global $acces_mandats_affaires_autres_negociateurs, $acces_negociateurs_classement_par_departement, $departements_autorises, $acces_acheteurs_recherches_autres_negociateurs;
    $tab = [];

    $clause = (isset($acces_acheteurs_recherches_autres_negociateurs) && $acces_acheteurs_recherches_autres_negociateurs) ? '1=1' : ' attribue_a = ' . $_SESSION[_USER_]->id;

    $model->release();
    $model->setChamp("r.id, r.reference, a.nom, a.prenom, a.raison_sociale, a.rcs_siret, a.code_type_profil");
    $model->settable($nom_bd . '.crm_np_acheteur_recherche r,' . $nom_bd . '.crm_np_acheteur a');
    $model->setClause($clause . ' AND r.id_acheteur=a.id');
    ...
    $clause = (isset($acces_mandats_affaires_autres_negociateurs) && $acces_mandats_affaires_autres_negociateurs) ? '1=1' : ' attribue_a = ' . $_SESSION[_USER_]->id;
    if (isset($acces_negociateurs_classement_par_departement) && $acces_negociateurs_classement_par_departement) {
        $clause .= isset($departements_autorises) ? ' AND num_departement IN ("' . implode('","', $departements_autorises) . '") ' : '';
    }
    $model->setChamp("a.id, a.reference, a.enseigne, a.code_type_transaction, a.id_type_affaire, a.adresse, t.libelle, v.nom_reel");
    $model->settable($nom_bd . '.crm_np_affaire a, ' . $nom_bd . '.crm_np_affaire_type t, crm_np_ville v');
...
function saveNotification($model, $route, $codeNotification, $concerne, $toReplace = [], $acces = false, $idUserConcerne = null, $infosUser = [])
{
    ...
    if (isset($_SESSION[_USER_])) {
        $infosUser['id'] = $_SESSION[_USER_]->id;
        $infosUser['id_agence'] = $_SESSION[_USER_]->id_agence;
        $infosUser['nom_dossier'] = $_SESSION[_USER_]->nom_dossier;
    }
    ...
    if (count($not) && count($infosUser)) {
        ...
        foreach ($usersNotif as $idUser => $valIdUser) {
            $filePath = $route . STORAGE_PATH . 'agences/' . $infosUser['nom_dossier'] . '/json/notif_user_' . $valIdUser;
        ...
        $filePath_G = $route . STORAGE_PATH . 'agences/' . $infosUser['nom_dossier'] . '/json/notif_general';
...
{
    $notifs = JSONFileToarray($route . STORAGE_PATH . 'agences/' . $_SESSION[_USER_]->nom_dossier . '/json/notif_user_' . $_SESSION[_USER_]->id);
    return $notifs;
}
...
function docfromcourrier($model, $clause)
{
    ...
    $model->release();
    $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_courrier_envoye');
    ...
    foreach ($envoyes as $v) {
        ...
        foreach ($destinataires as $destinataire) {
            $pathcourriersenvoyes = getdir($route . STORAGE_PATH, "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/courriersenvoyes/lettrestype/" . $v->id . "/$destinataire/");
            ...
            $folder = getdir($route . STORAGE_PATH, "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/courriersenvoyes/lettrestype/piecesjointes/" . $content["piecesjointes"] . "/");
            $file = getFiles($folder, $truncature, true, ['pdf'], 'file');
            if (!isset($file[0])) {
                continue;
            }
            $datas['envoyes'][$k]["files"] = pathinfo($file[0]);
            $datas['envoyes'][$k]["files"]["httppath"] = HTTP_PATH . STORAGE_PATH . "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/courriersenvoyes/lettrestype/piecesjointes/" . $content["piecesjointes"] . "/" . $datas['envoyes'][$k]["files"]["basename"];
    ...
    $model->release();
    $model->setTable($_SESSION[_USER_]->nom_bd . '.crm_np_courrier_imprime');
    ...
    foreach ($imprimes as $v) {
        ...
        foreach ($destinataires as $destinataire) {
            ...
            $pathcourriersimprimes = getdir($route . STORAGE_PATH, "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/courriersimprimes/" . $v->pid . "/" . $v->id . "/$destinataire/");
            ...
            $datas['imprimes'][$k]["files"]["httppath"] = HTTP_PATH . STORAGE_PATH . "agences/" . $_SESSION[_USER_]->nom_dossier . "/documents/courriersimprimes/" . $v->pid . "/" . $v->id . "/$destinataire/" . $datas['imprimes'][$k]["files"]["basename"];
...
function historyNavigation($titre = "", $picto = "fas fa-history")
{
    ...
    if (isset($_SESSION[_USER_]->history)) {
        $found_key = array_search($target, array_column($_SESSION[_USER_]->history, 'target'));

        if ($found_key === false) {
            if (count($_SESSION[_USER_]->history) >= 8) {
                unset($_SESSION[_USER_]->history[0]);
            }
        } else {
            unset($_SESSION[_USER_]->history[$found_key]);
        }
    }

    $_SESSION[_USER_]->history[] = array('target' => $target, 'lien' => $actual_link, 'titre' => $titre, 'picto' => $picto);
    $_SESSION[_USER_]->history = array_values($_SESSION[_USER_]->history);
}
